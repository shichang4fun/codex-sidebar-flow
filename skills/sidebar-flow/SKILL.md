...
3. Explain that lifecycle Hooks are observation-only because matching command Hooks run concurrently. A remote host must pass a trusted app-tools pipe capability probe before installation; otherwise use heartbeat self-heal. Never claim real-time sidebar mutation without a verified post-outcome bridge.
4. Only create or change a recurring heartbeat when the user explicitly requests self-healing. Render `docs/heartbeat-prompt.md` with `scripts/render-heartbeat.mjs --exclude <organizer-task-id>` and verify that the resulting prompt contains every exact excluded task ID and no placeholder. The heartbeat must not run the native-pipe script from a sandboxed shell.
